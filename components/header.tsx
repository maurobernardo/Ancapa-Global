...
import { Menu, X, Home, LayoutGrid, MapPinned, Briefcase, Newspaper, ChevronDown, ArrowRight } from "lucide-react";
...
export function Header() {
  ...
  return <header className="sticky top-0 z-50 border-b border-slate-200/80 bg-white/90 backdrop-blur-xl">
    ...
    {open && <div className="lg:hidden border-t bg-white px-6 py-5 flex flex-col gap-4">
      ...
      {navLinks.slice(1).map(([label, href, Icon])=><Link key={href} href={href} onClick={()=>setOpen(false)} className={`flex items-center gap-2.5 rounded-xl px-3 py-2 -mx-3 ${pathname===href?"bg-navy text-white font-semibold":"text-slate-700"}`}><Icon className="h-4 w-4"/>{label}</Link>)}
      <Link href="/contact" onClick={()=>setOpen(false)} className="btn btn-dark justify-center mt-2">Partner with ANCAPA</Link>
    </div>}
  </header>
}
