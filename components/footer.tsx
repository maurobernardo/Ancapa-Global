...
export function Footer(){
 return <footer className="bg-[#071c2c] text-white">
   <div className="container py-14 grid gap-10 md:grid-cols-[1.3fr_.8fr_.8fr_.9fr]">
     ...
     <div><h3 className="text-sm font-semibold mb-4">Contact</h3><div className="space-y-3 text-sm text-slate-300"><a href={`mailto:${company.email}`} className="flex items-center gap-2 hover:text-white transition"><Mail className="h-4 w-4 shrink-0"/>{company.email}</a><Link href="/contact" className="block hover:text-white transition">Start a conversation →</Link></div></div>
   </div>
